...
import { BB_MAX_LEVEL_IDX, MAX_LEVEL_IDX } from '../src/lib/cpm';
...
const raw = JSON.parse(fs.readFileSync(OUT, 'utf8')) as {
  ...
  species: { id: string; leagues: string[]; shadowLeagues: string[]; bestIv?: Record<string, number>; bestIvBB?: Record<string, number> }[];
};
...
for (const row of raw.species) {
  ...
  const search = (league: (typeof LEAGUES)[number], maxIdx: number) => {
    ...
    return ivKey({ a: best!.a, d: best!.d, s: best!.s });
  };

  const bestIv: Record<string, number> = {};
  // Second index for the Best Buddy ceiling. Opponents are always priced at
  // that ceiling, so without this every eligible opponent fell back to the
  // 4096 search and the precomputed index bought nothing on the path that
  // actually runs.
  const bestIvBB: Record<string, number> = {};
  for (const lg of leagues) {
    const league = LEAGUES.find((l) => l.id === lg)!;
    bestIv[lg] = search(league, MAX_LEVEL_IDX);
    const bb = search(league, BB_MAX_LEVEL_IDX);
    if (bb !== bestIv[lg]) bestIvBB[lg] = bb;
  }
  row.bestIv = bestIv;
  if (Object.keys(bestIvBB).length) row.bestIvBB = bestIvBB;
  written++;
}
